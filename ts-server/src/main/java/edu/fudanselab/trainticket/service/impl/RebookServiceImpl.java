...
import edu.fudanselab.trainticket.entity.PaymentDifferenceInfo;
...
public class RebookServiceImpl implements RebookService {
    ...
    public Response rebook(RebookInfo info, HttpHeaders httpHeaders) {
        ...
        Response<CommonOrder> queryOrderResult = getOrderByRebookInfo(info, httpHeaders);

        if (queryOrderResult.getStatus() == 1) {
            if (queryOrderResult.getData().getStatus() != 1) {
                RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder not suitable to rebook][OrderId: {}]",info.getOrderId());
                return new Response<>(0, "you order not suitable to rebook!", null);
            }
        } else {
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder not found][OrderId: {}]",info.getOrderId());
            return new Response(0, "order not found", null);
        }

        CommonOrder order = queryOrderResult.getData();
        int status = order.getStatus();
        if (status == OrderStatus.NOTPAID.getCode()) {
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder not paid][OrderId: {}]",info.getOrderId());
            ...
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder can't change twice][OrderId: {}]",info.getOrderId());
            ...
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder already collected][OrderId: {}]",info.getOrderId());
            return new Response<>(0, "You have already collected your ticket and you can change it now.", null);
        } else {
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder can't change][OrderId: {}]",info.getOrderId());
        ...
        if (!checkTime(order.getTravelDate(), order.getTravelTime())) {
            RebookServiceImpl.LOGGER.warn("[rebook][Rebook warn][CommonOrder beyond change time][OrderId: {}]",info.getOrderId());
        ...
        if (priceOld.compareTo(priceNew) > 0) {
            ...
            CommonOrder orderMoneyDifference = new CommonOrder();
    ...
    public Response payDifference(RebookInfo info, HttpHeaders httpHeaders) {
        ...
        CommonOrder order = (CommonOrder) queryOrderResult.getData();
    ...
    private Response updateOrder(CommonOrder order, RebookInfo info, TripAllDetail gtdr, String ticketPrice, HttpHeaders httpHeaders) {
        ...
        if ((tripGD(oldTripId) && tripGD(info.getTripId())) || (!tripGD(oldTripId) && !tripGD(info.getTripId()))) {
            ...
            if (changeOrderResult.getStatus() == 1) {
                ...
                return new Response<>(0, "Can't update CommonOrder!", null);
            }
        } else {
    ...
    private Response<CommonOrder> getOrderByRebookInfo(RebookInfo info, HttpHeaders httpHeaders) {
        ...
        ResponseEntity<Response<CommonOrder>> reGetOrderByRebookInfo = restTemplate.exchange(
                requestUrl,
                HttpMethod.GET,
